...
function handleMouseOver(e, d) {
    var name = Object.keys(d).includes('title') ? d.title : false;
    if(name!==false){
...
function handleMouseLeave(e, d) {
    ...
    const colorScale = d3
        .scaleLinear()
        .range(['#86c2ff', '#08306b'])
        .domain(d3.extent(dataset.map((d) => +d['nr_of_movies'])));
    ...
    const geo = d3
        ...
        .style('fill', (d) => {
            ...
            if (country.length === 0) {
                ...
                return colorScale(country[0]['nr_of_movies']);
            }
        });
    ...
    //             var country = dataset.filter((da) => da.country === n);
    //             if (country.length === 0) {
    //                 color = lb_lightGrey;
    //             } else {
...
function changeColor(d, color) {
    ...
    if(Array.isArray(name) === true){
        ...
        forEach((n) => {     
            ...
            .selectAll('path')
            .filter((c) => {
                if (n === c.properties.name) return c;
            })
            .style('fill', color);
            const circles = d3.select('div#scatter').selectAll('circle');
